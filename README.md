...
运行方式有两种，一为直接在IDEA中点击LoginTestApplicationTests中类定义或者各方法定义的左侧的三角符号运行，另一为在IDEA终端输入命令./mvnw verify -Dtest=LoginTestApplicationTests(其实直接用mvn clean test就行)或者mvn clean test -Dtest=LoginTestApplicationTests(同理，直接用mvn clean test就行)

测试报告生成，由于我是在Maven里设置Allure2.29.0,电脑全局设置的Allure2.37.0，项目里下载了Allure3.3.1，我建议直接用npx allure serve src/test/test_results/allure-results(Node.js 命令)这样就可以使用Allure3了，不然就用mvn io.qameta.allure:allure-maven:serve(Maven命令)使用的是Allure2
